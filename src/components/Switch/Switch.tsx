...
const Switch: FC<Props> = props => {
    const {left, right, onChange, checked, className, ...attr} = props
    ...
    return (
        <div className={`switch switch--horizontal ${className}`}>
            <input id="on" type="radio" name="switch" value={left} onChange={_onChange} checked={currentSide === left}/>
            <label htmlFor="on">{left}</label>
            <input id="off" type="radio" name="switch" value={right} onChange={_onChange} checked={currentSide === right}/>
